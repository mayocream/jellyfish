...
import { YStack, Button, Card } from 'tamagui';
...
const VideoPlayerScreen: React.FC<VideoPlayerScreenProps> = ({ 
        ...
        if (parsedUrl.queryParams && 'url' in parsedUrl.queryParams) {
          ...
          if (urlParam) {
            ...
            if (urlParam.startsWith('http') && (
              ...
              setError('Invalid video URL format. Please provide a valid video URL.');
              setVideoUrl(defaultUrl);
            }
          } else {
  ...
  return (
    <YStack flex={1} backgroundColor="$background" padding="$4">
      <YStack flexDirection="row" justifyContent="space-between" alignItems="center" paddingBottom="$4">
        {onGoBack && (
          <Button
            size="$4" 
            circular
      ...
      {error ? (
        ...
        <View style={styles.loadingContainer}>
          ...
          <Text style={styles.loadingText}>Loading video URL...</Text>
        </View>
      ) : (
        <>
          <Card padding="$2" marginBottom="$4">
            <Text style={styles.urlText}>Playing: {videoUrl}</Text>
          </Card>
          <VideoPlayer 
            videoUrl={videoUrl} 
            autoPlay={true}
